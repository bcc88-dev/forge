...
def ai_complete_with_fallback(messages, provider, model, stream_callback=None, cwd=None):
    # Try primary provider, then exactly ONE OpenRouter fallback on failure.
    # Cloud-only: skips local Ollama in fallback chain.
    config = get_config()
    temperature = config.get('temperature', 0.7)
    ...
    try:
        if stream_callback:
            return stream_ai_response(messages, model, stream_callback)
        else:
            return stream_ai_response(messages, model)
    except Exception as e:
        primary_error = e

    # Fallback: try ONE OpenRouter cloud model (skip local Ollama)
    console.print('[bold yellow]WARNING: Primary {0}/{1} failed -- trying cloud fallback...[/bold yellow]'.format(provider, model))

    import os, requests

    # Hardcoded cloud fallback order (free/cheap first)
    CLOUD_FALLBACKS = [
    ...
    if not key:
        console.print('[bold red]ERROR: No OpenRouter API key -- cannot fallback[/bold red]')
        return 'Error: ' + str(primary_error)

    # Try ONE fallback (rate-limit conservative)
    fb_provider, fb_model = CLOUD_FALLBACKS[0]
    if fb_provider == provider and fb_model == model:
        fb_provider, fb_model = CLOUD_FALLBACKS[1]

    console.print('[dim]  trying {0}/{1}...[/dim]'.format(fb_provider, fb_model))
    response_chunks = []
    def fb_callback(chunk):
        print(chunk, end='', flush=True)
        response_chunks.append(chunk)

    try:
        result = _call_openrouter(messages, fb_model, fb_callback, temperature, key)
        if result and not result.startswith('Error:'):
            console.print('[green]  SUCCESS: Fallback worked with {0}![/green]'.format(fb_model))
            return ''.join(response_chunks)
        else:
            console.print('[red]  FAILED: {0}/{1} failed -- giving up[/red]'.format(fb_provider, fb_model))
            return 'Error: ' + str(primary_error)
...
def _call_openrouter(messages, model, stream_callback, temperature, key):
    # Internal OpenRouter call with streaming.
    import json
    try:
        payload = {
            'model': model,
        ...
        r = requests.post(
            ...
            headers={
                ...
                'HTTP-Referer': 'https://nxterm.local',
                'X-Title': 'NxTerm - Free Coding Agent'
            },
            json=payload,
            stream=True,
            timeout=180
        ...
        if r.status_code != 200:
            return 'Error: OpenRouter returned ' + str(r.status_code)
...
def chat_loop(auto_apply: bool = False, initial_instruction: str = None):
    ...
    while True:
        ...
        try:
            ...
            response_chunks = ai_complete_with_fallback(chat.get_conversation(), model, streaming_callback, cwd)
